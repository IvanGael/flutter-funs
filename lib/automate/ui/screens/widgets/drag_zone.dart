...
class _MyHomeScreenDragZoneState extends State<MyHomeScreenDragZone> {
  ...
  void _takeScreenShot() async {
    ...
    //             quality: 60,
    //             name: "automate${DateTime.now()}");
    //         if (result["isSuccess"]) {
    //           print("Screenshot successfully performed!");
    //           _showSnackBar("Image Saved Succesfully ${result["filePath"]}");
    //         } else {
    //           print("Screenshot not performet!");
    //         }
    //       } else {
    ...
    screenshotController.capture().then((value) async {
          if (value != null) {
            ...
            if (result["isSuccess"]) {
              print("Screenshot successfully performed!");
              _showSnackBar("Screenshot successfully performed");
            } else {
              print("Screenshot not performet!");
            }
          } else {
...
class LinesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    context.read<DragProvider>().listOfDraggedStates.forEach((element) {
      ...
      textPainter.paint(
          canvas,
          Offset(dx, dy).translate(AutomateAppBar.etatSize.width / 2 - 5.0,
              -AutomateAppBar.etatSize.height * .9));
    });
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      startOffset = startOffset.translate(
          reverseOffsets ? 0.0 : AutomateAppBar.etatSize.width,
          -AutomateAppBar.etatSize.height * (reverseOffsets ? .6 : .7));
      endOffset = endOffset.translate(
          reverseOffsets ? AutomateAppBar.etatSize.width : 0.0,
          -AutomateAppBar.etatSize.height * (reverseOffsets ? .5 : 0.7));
